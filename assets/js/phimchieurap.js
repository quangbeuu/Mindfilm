...
const displayFilm = (film) => {
    let phimchieurap = film['phimchieurap']
    let htmlString = phimchieurap
        .map((phimchieurap) =>{
            return `
            <div class="movie__item">
                <img src="${phimchieurap.imageUrl}" alt="" class="movie__image">
                <h1 class="movie__title">${phimchieurap.title}</h1>
            </div>
            `
        })
    document.getElementById("phimchieurap").innerHTML = htmlString;
}
